...
void process_cmd(int client_fd, client_info *info) {
	if (info->cmd == GET) {
		int len = strlen(dir_name) + strlen(info->filename) + 2;
		char path[len];
		memset(path,0, len);
...
int read_body(int client_fd, client_info *info) {
	...
	while (bytes_read < size+5) {
		size_t remaining_bytes = size + 5 - bytes_read;
        size_t size_hd = (remaining_bytes > 1024) ? 1024 : remaining_bytes;
