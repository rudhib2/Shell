 ...
 *
 * This file implements supporting code for mini_memcheck.
 *
 * You shouldn't modify any of it, nor are you expected to understand all of
 * it, but you are welcome to study it to figure out how mini_memcheck works.
 *
 * ----------------------------------------------------------------------------
 */
...
static void fetch_caller_info(const char **filename, void **instruction) {
    ...
    static const char *internal = "mini_memcheck";
...
static void resolve_addr2line(char *symbol, char *source, const char *filename,
    ...
    if (!child) {
        ...
        char *command = "llvm-addr2line-14";
...
static void resolve(char *output, const char *filename, void *instruction,
    ...
    void *offset = (void *)((long)instruction - (long)info.dli_fbase);
    resolve_addr2line(symbol, source, filename, offset);
...
/*
 * On shared library unload, print the mini_memcheck leak report.
 */
