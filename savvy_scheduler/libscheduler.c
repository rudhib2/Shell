...
int comparer_fcfs(const void *a, const void *b) {
    ...
    if (info_a->arrive_time < info_b->arrive_time) return -1;
    return 1;
}
...
int comparer_pri(const void *a, const void *b) {
    ...
    job_info *info_a = ((job*)a)->metadata;
    job_info *info_b = ((job*)b)->metadata;
    if ((info_a->priority - info_b->priority) == 0) return break_tie(a, b);
    else if ((info_a->priority - info_b->priority) < 0) return -1;
    return 1;
}
...
int comparer_psrtf(const void *a, const void *b) {
    ...
    job_info *info_a = ((job*)a)->metadata;
    job_info *info_b = ((job*)b)->metadata;
    if ((info_a->remaining_time - info_b->remaining_time) == 0) return break_tie(a, b);
    else if ((info_a->remaining_time - info_b->remaining_time)<0) return -1;
    return 1;
}
...
int comparer_rr(const void *a, const void *b) {
    ...
    job_info *info_a = ((job*)a)->metadata;
    job_info *info_b = ((job*)b)->metadata;
    if ((info_a->recent_run_time - info_b->recent_run_time) == 0) return break_tie(a, b);
    else if ((info_a->recent_run_time - info_b->recent_run_time) < 0) return -1;
    return 1;
}
...
int comparer_sjf(const void *a, const void *b) {
    ...
    job_info *info_a = ((job*)a)->metadata;
    job_info *info_b = ((job*)b)->metadata;
    if ((info_a->required_time - info_b->required_time) == 0) return break_tie(a, b);
    else if ((info_a->required_time - info_b->required_time) < 0) return -1;
    return 1;
}
...
job *scheduler_quantum_expired(job *job_evicted, double time) {
    // TODO: Implement me!
    if (!job_evicted) return priqueue_peek(&pqueue);
    job_info* info = job_evicted->metadata;
    info->recent_run_time = time;
    info->remaining_time -= 1;
    if (info->start_time < 0) info->start_time = time -1;
