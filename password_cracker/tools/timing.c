...
int main(int argc, __attribute__((unused)) char **argv) {
    ...
    // do some busy work for a while, the CPU time and wall clock time will be
    // the same!
    ...
    // start 4 threads to do some work. On the VMs, there should be 2 cores.
    // This means we should see: 4*wall clock time ~= cpu time
    start_wall_time = getTime();
    start_cpu_time = getCPUTime();
