...
/**
 * In this assignment, we will have 32-bit virtual memory and physical memory
 * spaces. There will be two levels of page tables in this system, and each
 * page table will be 4kb (2^12 bytes) in size.
 */
 ...
 * variable represents, see the comments below. You may want to Google to know
...
/**
 * The definition of a page directory. Note that we implement it as an array
 * of page directory entries. Each page directory entry will point to a page
 * table.
 *
...
/**
 * The layout of an entry in the page table. For details on what each variable
 * represents, see the comments below. You may want to Google to know more
 * about the : operator.
 */
typedef struct {
    uint32_t base_addr : 20;
...
/**
 * The definition of a page table. Note that we implement it as an array
